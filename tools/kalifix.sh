...
test_connection() {
  ...
  if command -v getent >/dev/null 2>&1; then
    getent hosts http.kali.org >/dev/null && echo -e "${GREEN}[✓] DNS resolution OK${NC}" \
      || echo -e "${RED}[!] DNS resolution failed${NC}"
  elif command -v host >/dev/null 2>&1; then
    host http.kali.org >/dev/null && echo -e "${GREEN}[✓] DNS resolution OK${NC}" \
      || echo -e "${RED}[!] DNS resolution failed${NC}"
  elif command -v nslookup >/dev/null 2>&1; then
    nslookup http.kali.org >/dev/null && echo -e "${GREEN}[✓] DNS resolution OK${NC}" \
      || echo -e "${RED}[!] DNS resolution failed${NC}"
  else
    echo -e "${YELLOW}[-] No DNS lookup tool found (install dnsutils or bind9-host)${NC}"
  fi
}
...
show_banner() {
  echo -e "${BLUE}================================================${NC}"
  echo -e "${BLUE} _  __    _    _     ___   _____ _____  __ ${NC}"
  echo -e "${BLUE}| |/ /   / \  | |   |_ _| |  ___|_ _\ \/ / ${NC}"
  echo -e "${BLUE}| ' /   / _ \ | |    | |  | |_   | | \  /  ${NC}"
  echo -e "${BLUE}| . \  / ___ \| |___ | |  |  _|  | | /  \  ${NC}"
  echo -e "${BLUE}|_|\_\/_/   \_\_____|___| |_|   |___/_/\_\ ${NC}"
  echo -e "${BLUE}                                           ${NC}"
  echo -e "${BLUE}================================================${NC}"
  echo -e "${GREEN}   Kali Linux Repository Auto-Fixer v2.2${NC}"
  echo -e "   Contributed by: bitpy2023"
  echo -e "   GitHub: https://github.com/bitpy2023/Scripts"
  echo -e "   Last Updated: 2026-09-13"
  echo "----------------------------------------------"
}
